	...
	tuple_count(0)
{
	...
	assert(p_relation);
	max_tuple_count = relation->getSchema().getTuplesPerBlock();
}
...
vector<Tuple> HeapBlock::getTuples(MainMemory & memory)
{
	if(dStart < dEnd) {
		p_relation->getBlock(dStart++, mm_index);
		p_block = memory.getBlock(mm_index);
...
void HeapManager::_init(const vector<int>& diskHeadPtrs, MainMemory& memory)
{
	resetMemory();
	...
	for(int i = 0; i < sublists; ++i){
		...
		free_blocks.pop();
		heap_blocks.push_back(HeapBlock(p_relation, diskHeadPtrs[i], memory_block_index));
	}

	// 2. for each heap blocks, read one blocks from disk through memory into the heap
	for(int i = 0; i < heap_blocks.size(); ++i){
		HeapBlock & heap_block = heap_blocks[i];
		if(!heap_block.isExhausted()){
			vector<Tuple> tuples = heap_block.getTuples(memory);
...
vector<Tuple> HeapManager::popTopMins(MainMemory & memory){
...
void HeapManager::pop(MainMemory & memory){
...
Relation * Algorithm::runUnary(Relation * p_relation, MainMemory & memory, SchemaManager & schema_mgr, bool is_leaf)
{
	assert(p_relation);
	...
	string new_relation_name = p_relation->getRelationName() + T[m_type] + to_string(m_level) + to_string(_g_relation_counter++);
	Relation * newRelation = schema_mgr.createRelation(new_relation_name, getNewSchema(p_relation, is_leaf));
	assert(p_relation && newRelation);

	if(T[m_type] == "SELECT"){
	        Select(p_relation, newRelation, memory, false);
	}
	else if(T[m_type] == "PROJECT"){
		vector<int> indices = getNeededFields(p_relation->getSchema(), m_conditions);
		Project(p_relation, newRelation, memory, indices);
	}
	else if(T[m_type] == "DISTINCT"){
		if(m_isOnePass)
			distinctOnePass(p_relation, newRelation, memory);
		else
		    distinctTwoPass(p_relation, newRelation, memory, schema_mgr); 
		    // for both two-pass, the new relation is passed by reference!!!
	}
	else if(T[m_type] == "SORT"){
		if(m_isOnePass)
			sortOnePass(p_relation, newRelation, memory);
		else
			sortTwoPass(p_relation, newRelation, memory, schema_mgr);
	}
	else if(T[m_type] == "DELETE"){
	        Select(p_relation, newRelation, memory, true);
	}
	else{
...
Schema Algorithm::getNewSchema(Relation * p_relation, bool is_leaf)
{
	// old schema
	Schema old_schema = p_relation->getSchema();
	...
	if (is_leaf){
		for(int i = 0; i < indices.size(); ++i){
			old_names[indices[i]] = p_relation->getRelationName() + "." + old_names[indices[i]];
		}
	}
...
void Algorithm::Select(Relation * oldR, Relation * newR, MainMemory& memory, bool isInvert)
{
	assert(!free_blocks.empty());
	// get a available memory block
	int memory_block_index = free_blocks.front();
	free_blocks.pop();
	...
	while(size < dBlocks){
		...
		vector<Tuple> tuples = p_block->getTuples();
		if(tuples.empty()){
			cerr<<"Warning: No tuples in the current memory block!"<<endl;
		}
		for(int i = 0; i < tuples.size(); ++i){
			Tuple t = tuples[i];
			...
			appendTupleToRelation(newR, memory, tmp);
		}
		size++;
	}
...
void Algorithm::Project(Relation * oldR, Relation * newR, MainMemory& memory, vector<int> indices)
{
	assert(!free_blocks.empty());
	if(indices.empty()){
...
void Algorithm::distinctOnePass(Relation * oldR, Relation * newR, MainMemory & memory)
{
	assert(!free_blocks.empty());
	// get a available memory block
	int memory_block_index = free_blocks.front();
	...
	assert(dBlocks <= memory.getMemorySize()); // doing one pass here!
	...
	while(size < dBlocks){
		...
		vector<Tuple> tuples = p_block->getTuples();
		if(tuples.empty()){
			cerr<<"Warning: DistinctOnPass, No tuples in the current memory block!"<<endl;
		}
		...
		for(int i = 0; i < tuples.size(); ++i){
			...
			if(m_set.find(t) == m_set.end()){
				m_set.insert(t);
				appendTupleToRelation(newR, memory, t);
			}
		}
		size++;
	}
...
void Algorithm::distinctTwoPass(Relation * oldR, 
	Relation * & newR, MainMemory & memory, SchemaManager & schema_mgr)
{  
	int dBlocks=  oldR->getNumOfBlocks();
	const int mSize = memory.getMemorySize();
	...
	for(start = 0; start < dBlocks; start += cnt){
		...
		cnt = sortByChunkWB(oldR, newR, memory, start, inds);
		sublists++;
	}
	...
	// @param1:relation ptr, @param2: memory, @param3: head of the sublists, 
	// @param4: the vector of multiple/single target sort column
	HeapManager heapMgr(newR, memory, subHeads, inds);

	while(!heapMgr.empty()){
	  vector<Tuple> tuples = heapMgr.popTopMins(memory); 
	  appendTupleToRelation(newRR, memory, tuples[0]);
	}
	resetMemory();
...
int Algorithm::getNeededOrder(Relation * p_relation)
{
	// get the schema:
	Schema schema = p_relation->getSchema();
...
int Algorithm::sortByChunkWB(Relation * oldR, Relation * newR, 
	MainMemory & memory, int start, vector<int> indices)
{
	assert(!free_blocks.empty());
	// get a available memory block
	resetMemory();
	...
	for(int i = start; i < dBlocks && cnt < memory.getMemorySize(); i++, cnt++){
		assert(!free_blocks.empty());
		int memory_block_index = free_blocks.front();
		free_blocks.pop();
		...
		vector<Tuple> tuples = p_block->getTuples();
		if(tuples.empty()){
			cerr<<"Warning: sortByChunkWB, No tuples in the current memory block!"<<endl;
		}
	...
	for(int i = 0; i < sortListInt.size(); ++i){
		...
		appendTupleToRelation(newR, memory, sortListInt[i].second); 
	}

	for(int i = 0; i < sortListStr.size(); ++i){
		// always append to the end of the new relation!
		appendTupleToRelation(newR, memory, sortListStr[i].second); 
	}

	return cnt;
}

void Algorithm::sortOnePass(Relation * oldR, Relation * newR, MainMemory & memory)
{
	int dBlocks=  oldR->getNumOfBlocks();
	assert(dBlocks <= memory.getMemorySize()); // doing one pass here!
	...
	int cnt = sortByChunkWB(oldR, newR, memory, 0, vector<int>());
	assert(cnt == dBlocks);
	return;

}

void Algorithm::sortTwoPass(Relation * oldR, Relation *& newR, MainMemory & memory, SchemaManager& schema_mgr)
{
	int dBlocks=  oldR->getNumOfBlocks();
	const int mSize = memory.getMemorySize();
	if(sqrt(dBlocks) > double(mSize)){
	...
	for(start = 0; start < dBlocks; start += cnt){
		...
		cnt = sortByChunkWB(oldR, newR, memory, start, vector<int>());
		sublists++;
	}
	...
	// @param1:relation ptr, @param2: memory, @param3: head of the sublists, 
	// @param4: the vector of multiple/single target sort column
	HeapManager heapMgr(newR, memory, subHeads, vector<int>(1, indField));

	while(!heapMgr.empty()){
		vector<Tuple> tuples = heapMgr.popTopMins(memory);
		for(int j = 0; j < tuples.size(); ++j){
		  appendTupleToRelation(newRR, memory, tuples[j]);
		}
	}
	resetMemory();
...
vector<pair<string, string> > Algorithm::findJoinField()
{
	...
	for (int i = 0; i < m_conditions.size(); i++){
		vector<string> column = split(m_conditions[i], ".");
		// column name
		if (column.size() == 2){
		...
		// operator: only "=" "AND" is allowed
			...
			if (m_conditions[i] == "="){
				...
				if (split(op1_column, ".").size() == 2)
					op1_column = split(op1_column, ".")[1];

				string op2_column = op2;
				if (split(op2_column, ".").size() == 2)
					op2_column = split(op2_column, ".")[1];
...
Schema Algorithm::getJoinSchema(Relation *left, 
	Relation *right, bool left_is_leaf, bool right_is_leaf, 
	vector<vector<int> > &mapping, bool &is_natural)
{
			...
			for (int i = 0; i < names.size(); i++){
				...
				if (is_match){
					...
					if (field_idx[j] == -1){
						...
						assert(split(field, ".").size() == 2);
						if (split(field, ".").size() == 2)
							field = split(field, ".")[1];
						new_names[idx] = field;
...
Relation * Algorithm::runBinary(Relation * left, 
	Relation * right, MainMemory & memory, 
	SchemaManager & schema_mgr, bool left_is_leaf, bool right_is_leaf)
{
	...
	if (left_size > right_size) 
		return runBinary(right, left, memory, schema_mgr, right_is_leaf, left_is_leaf);
	...
	if (is_natural && cost2pass < cost1pass){
		...
		join2Pass(left, right, idx_map[0], idx_map[1], join_relation, memory, schema_mgr);
	}

	else{
		//cout<<"Using 1 pass join!"<<endl;
		join1Pass(left, right, idx_map[0], idx_map[1], join_relation, memory);
	}
	return join_relation;
}

vector<int> Algorithm::subSortForJoin(Relation* oldR, Relation* &newR, 
	MainMemory &memory, SchemaManager &schema_mgr, vector<int> indices)
{
	...
	for(int start = 0; start < dBlocks; start += cnt){
		subHeads.push_back(start);
		// sort the subchunk first and write back
		cnt = sortByChunkWB(oldR, newR, memory, start, indices);
		sublists++;
	}
	return subHeads;
}

void Algorithm::join2Pass(Relation *old_left, Relation *old_right, 
	vector<int> left_map, vector<int> right_map, Relation *join, 
	MainMemory& memory, SchemaManager &schema_mgr)
{
	...
	// first pass
	vector<int> leftHeads = subSortForJoin(old_left, new_left, memory, schema_mgr, join_field_left);
	vector<int> rightHeads = subSortForJoin(old_right, new_right, memory, schema_mgr, join_field_right);


	// second pass, merge the sorted sublists, note that the to-be-merged one is in the newR  
	// @param1:relation ptr, @param2: memory, @param3: head of the sublists, 
	// @param4: the vector of multiple/single target sort column
	HeapManager leftHMgr(new_left, memory, leftHeads, join_field_left);
	HeapManager rightHMgr(new_right, memory, rightHeads, join_field_right);

	while(!leftHMgr.empty() || !rightHMgr.empty()){
		vector<Tuple> left_tuples = leftHMgr.popTopMins(memory);
		vector<Tuple> right_tuples = rightHMgr.popTopMins(memory);
		for(int i = 0; i < left_tuples.size(); ++i){
			...
			for (int j = 0; j < right_tuples.size(); ++j){
				...
				appendTupleToRelation(join, memory, join_tuple);
			}
		}
	}
}

void Algorithm::join1Pass(Relation *left, Relation *right, vector<int> left_map, vector<int> right_map, Relation *join, MainMemory& memory)
{
	int num_blocks = free_blocks.size();
	int left_size =  left->getNumOfBlocks();
	...
	while (left_idx < left_size){
		...
		for (int j = 0; j < right_size; j++){
			...
			Block *right_ptr = memory.getBlock(p_right);
			...
			for(int r = 0; r < right_tuples.size(); ++r){
				...
				for (int i = 0; i < p_left_in_use; i++){
					Block *left_ptr = memory.getBlock(p_left[i]);
